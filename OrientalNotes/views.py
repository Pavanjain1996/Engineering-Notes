...
def upload(request):
   ...
   reciever = []
   for i in data:
      reciever.append(i.email)
   email = EmailMessage('Content Uploaded','We have uploaded '+content_type+' for '+branch+' branch and '+semester+' semester for the subject '+subject,to=reciever)
   email.send()
   return render(request,'Upload.html')
